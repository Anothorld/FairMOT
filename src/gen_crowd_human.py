...
for line in tqdm(f.readlines()):
    ...
    for inst in anno['gtboxes']:
        if inst['tag'] != 'person':
            continue
        vbox = [inst['vbox'][0] / width, inst['vbox'][1] / height, inst['vbox'][2] / width, inst['vbox'][3] / height]
        if 'ignore' in inst['head_attr'].keys():
            if inst['head_attr']['ignore'] == 1:
                hbox = [-1, -1, -1, -1]
            else:
                hbox = [inst['hbox'][0] / width if inst['hbox'][0] / width > 0 else 0, inst['hbox'][1] / height if inst['hbox'][1] / height > 0 else 0, inst['hbox'][2] / width, inst['hbox'][3] / height]
